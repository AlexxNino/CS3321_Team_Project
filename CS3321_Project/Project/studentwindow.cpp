...
void StudentWindow::get_semester_gpa(){
    ...
    {
        qry_gpa.exec("UPDATE students set gpa = (SELECT avg( Case WHEN test_results >= 90 THEN 4 WHEN test_results > 79 and test_results < 90 THEN 3 WHEN test_results > 69 and test_results < 79 then 2 WHEN test_results > 59 and test_results < 69 then 1 ELSE 0 END)from student_grade WHERE registration_id = (SELECT registration_id from student_course_enrollment where student_id = (SELECT student_id from students where username = '"+Global_Username+"' AND password = '"+Global_Password+"')))WHERE student_id = (SELECT student_id from students where username = '"+Global_Username+"' AND password = '"+Global_Password+"')");
...
/*
anino19     Password123
jlozano07	Password123
jheymen32	Password123
radnan24	Password123
cmora15     Password123
*/
